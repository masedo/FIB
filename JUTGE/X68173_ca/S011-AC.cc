...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est) {
  ...
  double nota_max_antiga = -1;
  if (vest[i].te_nota())
     nota_max_antiga = vest[i].consultar_nota();
  
  vest[i] = est;
  
  if (imax == i) {
    if ((est.te_nota() and est.consultar_nota() < nota_max_antiga) or (not est.te_nota()))
...
void Cjt_estudiants::modificar_iessim(int i, const Estudiant& est) {
  ...
  double nota_max_antiga = -1;
  if (imax == i-1 and vest[i-1].te_nota())
     nota_max_antiga = vest[i-1].consultar_nota();
...
void Cjt_estudiants::esborrar_estudiant(int dni) {
  bool trobat = false, recalc = false;
  for (int i = 0; i < nest-1; ++i) {
        
    if (vest[i].consultar_DNI() == dni) {
      trobat = true;
      if (imax == i) recalc= true;
      else if (imax > i) --imax;
    }
    
    if (trobat) vest[i] = vest[i+1];
  }
  --nest; 
  if (recalc or imax == nest) recalcular_posicio_imax();
}
